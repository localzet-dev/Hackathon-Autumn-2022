...
class OAuthRequest
{
    ...
    {
        $http_url = ($http_url) ? $http_url : request()->header('x-forwarded-proto', 'https') . request()->url();
        $http_method = ($http_method) ? $http_method : request()->method();
        ...
        if (!$parameters) {
            ...
            $parameters = OAuthUtil::parse_parameters(request()->queryString());

            // It's a POST request of the proper content-type, so parse POST
            // parameters and add those overriding any duplicates from GET
            if ($http_method == "POST" && request()->header('content-type', false) && strstr(request()->header('content-type'), 'application/x-www-form-urlencoded')) {
                $post_data  = OAuthUtil::parse_parameters(request()->rawBody());
                $parameters = array_merge($parameters, $post_data);
            }

            // We have a Authorization-header with OAuth data. Parse the header
            // and add those overriding any duplicates from GET or POST
            if (request()->header('authorization', false) && substr(request()->header('authorization'), 0, 6) == 'OAuth ') {
                $header_parameters = OAuthUtil::split_header(request()->header('authorization'));
